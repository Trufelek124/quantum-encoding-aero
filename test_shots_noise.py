...
def parse_args():
    ...
    p.add_argument("--smooth-channels", type=int, default=64)
    p.add_argument("--smooth-layers", type=int, default=4)
    p.add_argument("--use-wavelet", action="store_true")
    p.add_argument("--shots-levels", type=int, nargs="+", default=[1024, 256, 64],
                    help="К-сть вимірів (shots) на прогін кола. Менше = реалістичніше "
                         "(дешевше залізо) і зашумленіше. None (нескінченно) = ідеальний "
                         "симулятор, з яким ми тренували.")
    p.add_argument("--out-dir", type=str, default="shots_test")
...
def main():
    ...
    model.eval()
    print(f"Модель завантажено: {args.checkpoint} (backend={backend_used})")
    ...
    print(f"\nІДЕАЛЬНИЙ СИМУЛЯТОР (shots=нескінченно): MAE={mae_clean:.4f}")

    panels = [
        ("1. ОРИГІНАЛ\n(повний розмір)", true_orig, None),
        (f"2. ВХІД МОДЕЛІ\n({image_size}x{image_size}, стиснуто)", to_pil(target[0]), None),
        ("3. ІДЕАЛЬНИЙ СИМУЛЯТОР", to_pil(recon_clean[0]), mae_clean),
    ]
    results = [("ideal (shots=inf)", mae_clean)]

    for i, shots in enumerate(args.shots_levels, start=4):
        print(f"\nБудую коло з shots={shots} ...")
        # lightning.qubit підтримує shots напряму -- НЕ треба default.mixed,
        # значно дешевше за симуляцію матриці густини.
        dev_shots = qml.device("lightning.qubit", wires=total_wires, shots=shots)
        qnode_shots = create_qnode(num_qubits, args.num_layers, dev_shots, diff_method="parameter-shift")
        model.quantum_layer = PennyLaneQuantumLayer(qnode_shots)

        with torch.no_grad():
            recon_shots = model(batch)
        mae_shots = torch.nn.functional.l1_loss(recon_shots, target).item()
        print(f"shots={shots}: MAE={mae_shots:.4f} (зростання проти ідеального: {mae_shots - mae_clean:+.4f})")

        panels.append((f"{i}. shots={shots}", to_pil(recon_shots[0]), mae_shots))
        results.append((f"shots={shots}", mae_shots))
    ...
    out_path = os.path.join(args.out_dir, "shots_comparison.png")
    combined.save(out_path)

    print(f"\n{'=' * 50}\nПІДСУМОК (вплив к-сті вимірів shots на якість):")
    for label, mae in results:
        print(f"  {label}: MAE={mae:.4f}")
    print(f"{'=' * 50}")
    print(f"Картинка: {out_path}")
